...
kubectl -n $NS delete cm envoy-config --ignore-not-found=true
kubectl -n $NS create cm envoy-config --from-file=envoy.yaml=/tmp/envoy.yaml

kubectl -n $NS rollout restart deploy/envoy
kubectl -n $NS rollout status deploy/envoy
echo "✅ forced known-good envoy config"
